...
// */
//public class SqlSessionPoolsOld implements ISqlSessionPools {
//
//    private static Logger log = Logger.getLogger(SqlSessionPoolsOld.class);
...
//
//    public static final int SESSION_INIT_NUM = 2;//每个连接的连接数量配置
...
//
//    static {
//        String path = SqlSessionPoolsOld.class.getClassLoader().getResource(CONF_FILE_NAME).getPath().substring(1);
//        try {
...
//        for (int i = 0; i < SqlSessionPoolsOld.SESSION_INIT_NUM; i++) {
...
//                //返回SqlSession对象的代理对象
//                return (SqlSession) Proxy.newProxyInstance(SqlSessionPoolsOld.class.getClassLoader(), session.getClass().getInterfaces(), new InvocationHandler() {
...
////        return new SqlSessionTemplate(getSqlSessionFactory(id));
...
//            InputStream is = SqlSessionPoolsOld.class.getClassLoader().getResourceAsStream(resource);
